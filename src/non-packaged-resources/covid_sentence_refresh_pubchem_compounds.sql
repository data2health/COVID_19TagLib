...
where doi is not null
  and exists (select doi from covid_pubchem.refresh_queue where sentence_compound_staging.doi=refresh_queue.doi)
  and not exists (select doi from covid_pubchem.sentence_compound where sentence_compound.doi=sentence_compound_staging.doi);
\echo -n 'by PMCID...   '
insert into covid_pubchem.sentence_compound select * from covid_pubchem.sentence_compound_staging
where pmcid > 0
  and exists (select pmcid from covid_pubchem.refresh_queue where sentence_compound_staging.pmcid=refresh_queue.pmcid)
...
insert into covid_pubchem.sentence_compound select * from covid_pubchem.sentence_compound_staging
where pmid > 0
  and exists (select pmid from covid_pubchem.refresh_queue where sentence_compound_staging.pmid=refresh_queue.pmid)
